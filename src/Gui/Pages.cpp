...
// measuresGrid = swimgearmeasuresGrid

SwimGearMeasuresPage::SwimGearMeasuresPage(QWidget *parent, Context *context, SwimGearMeasuresGroup *swimgearmeasuresGroup) : QWidget(parent), context(context), swimgearmeasuresGroup(swimgearmeasuresGroup)
{
    metricUnits = context->athlete->useMetricUnits;
    QList<double> unitsFactors = swimgearmeasuresGroup->getFieldUnitsFactors();

    QVBoxLayout *all = new QVBoxLayout(this);
    QGridLayout *swimgearmeasuresGrid = new QGridLayout;
    Qt::Alignment alignment = Qt::AlignLeft|Qt::AlignVCenter;
    ...
    QLabel* seperatorText = new QLabel(tr("Time dependent %1 swimgearmeasures").arg(swimgearmeasuresGroup->getName()));
    all->addWidget(seperatorText);
    ...
    swimgearmeasuresGrid->addWidget(dateLabel, 1, 0, alignment);
    swimgearmeasuresGrid->addWidget(dateTimeEdit, 1, 1, alignment);

    QStringList fieldNames = swimgearmeasuresGroup->getFieldNames();
    valuesLabel = QVector<QLabel*>(fieldNames.count());
    valuesEdit = QVector<QDoubleSpinBox*>(fieldNames.count());
    int k = 0;
    foreach (QString fieldName, fieldNames) {
        ...
        valuesEdit[k]->setDecimals(2);
        valuesEdit[k]->setValue(0.0);
        valuesEdit[k]->setSuffix(QString(" %1").arg(swimgearmeasuresGroup->getFieldUnits(k, metricUnits)));

        swimgearmeasuresGrid->addWidget(valuesLabel[k], k+2, 0, alignment);
        swimgearmeasuresGrid->addWidget(valuesEdit[k], k+2, 1, alignment);
    ...
    swimgearmeasuresGrid->addWidget(commentlabel, k+2, 0, alignment);
    swimgearmeasuresGrid->addWidget(comment, k+2, 1, alignment);

    all->addLayout(swimgearmeasuresGrid);
    ...
    // Measures
    swimgearmeasuresTree = new QTreeWidget;
    swimgearmeasuresTree->headerItem()->setText(0, dateTimetext);
    swimgearmeasuresTree->header()->setSectionResizeMode(0, QHeaderView::ResizeToContents);

    k = 0;
    foreach (QString fieldName, fieldNames) {
        swimgearmeasuresTree->headerItem()->setText(k+1, fieldName);
        swimgearmeasuresTree->header()->setSectionResizeMode(k+1, QHeaderView::ResizeToContents);
        k++;
    }

    swimgearmeasuresTree->headerItem()->setText(k+1, commenttext);
    swimgearmeasuresTree->header()->setSectionResizeMode(k+1, QHeaderView::ResizeToContents);
    swimgearmeasuresTree->headerItem()->setText(k+2, tr("Source"));
    swimgearmeasuresTree->header()->setSectionResizeMode(k+2, QHeaderView::ResizeToContents);
    swimgearmeasuresTree->headerItem()->setText(k+3, tr("Original Source"));
    swimgearmeasuresTree->setColumnCount(k+4);
    swimgearmeasuresTree->setSelectionMode(QAbstractItemView::SingleSelection);
    swimgearmeasuresTree->setEditTriggers(QAbstractItemView::SelectedClicked); // allow edit
    swimgearmeasuresTree->setUniformRowHeights(true);
    swimgearmeasuresTree->setIndentation(0);

    // get a copy of group swimgearmeasures if the file exists
    swimgearmeasures = QList<SwimGearMeasure>(swimgearmeasuresGroup->swimgearmeasures());

    // setup swimgearmeasuresTree
    for (int i=0; i<swimgearmeasures.count(); i++) {
        QTreeWidgetItem *add = new QTreeWidgetItem(swimgearmeasuresTree->invisibleRootItem());
        add->setFlags(add->flags() & ~Qt::ItemIsEditable);
        // date & time
        add->setText(0, swimgearmeasures[i].when.toString(tr("MMM d, yyyy - hh:mm:ss")));
        // swimgearmeasures data
        k = 0;
        foreach (QString fieldName, fieldNames) {
            const double unitsFactor = (metricUnits ? 1.0 : unitsFactors[k]);
            add->setText(k+1, QString("%1").arg(swimgearmeasures[i].values[k]*unitsFactor, 0, 'f', 2));
            k++;
        }

        add->setText(k+1, swimgearmeasures[i].comment);
        // swimgearsource
        add->setText(k+2, swimgearmeasures[i].getSourceDescription());
        add->setText(k+3, swimgearmeasures[i].swimgearoriginalSource);
    }

    all->addWidget(swimgearmeasuresTree);

    // set default edit values to newest swimgearmeasures (if one exists)
    if (swimgearmeasures.count() > 0) {
        for (int k = 0; k < fieldNames.count(); k++) {
            const double unitsFactor = (metricUnits ? 1.0 : unitsFactors[k]);
            valuesEdit[k]->setValue(swimgearmeasures.last().values[k]*unitsFactor);
        }
    }
    ...
    // list selection connect
    connect(swimgearmeasuresTree, SIGNAL(itemSelectionChanged()), this, SLOT(rangeSelectionChanged()));

    // save initial values for things we care about
    b4.fingerprint = 0;
    foreach (SwimGearMeasure swimgearmeasure, swimgearmeasures) {
        b4.fingerprint += swimgearmeasure.getFingerprint();
    }
}

void
SwimGearMeasuresPage::unitChanged(int currentIndex)
{
    metricUnits = (currentIndex == 0);
    QList<double> unitsFactors = swimgearmeasuresGroup->getFieldUnitsFactors();
    ...
    // update swimgearmeasuresTree
    for (int i=0; i<swimgearmeasuresTree->invisibleRootItem()->childCount(); i++) {
        QTreeWidgetItem *edit = swimgearmeasuresTree->invisibleRootItem()->child(i);

        for (int k = 0; k < valuesEdit.count(); k++) {
            const double unitsFactor = (metricUnits ? 1.0 : unitsFactors[k]);
            edit->setText(k+1, QString("%1").arg(swimgearmeasures[i].values[k]*unitsFactor, 0, 'f', 2));
        }
    }
}

qint32
SwimGearMeasuresPage::saveClicked()
{
    qint32 state=0;

    // Measures changed ?
    unsigned long fingerprint = 0;
    foreach (SwimGearMeasure swimgermeasure, swimgearmeasures) {
        fingerprint += swimgearmeasure.getFingerprint();
    }
    if (fingerprint != b4.fingerprint) {
        // store in athlete
        swimgearmeasuresGroup->setMeasures(swimgearmeasures);
        // now save data away if we actually got something !
...
void
SwimGearMeasuresPage::addOReditClicked()
{
    int index;
    QTreeWidgetItem *add;
    SwimGearMeasure addMeasure;
    QString dateTimeTxt = dateTimeEdit->dateTime().toString(tr("MMM d, yyyy - hh:mm:ss"));

    // if an entry for this date & time already exists, edit item otherwise add new
    QList<QTreeWidgetItem*> matches = swimgearmeasuresTree->findItems(dateTimeTxt, Qt::MatchExactly, 0);
    if (matches.count() > 0) {
        // edit existing
        add = matches[0];
        index = swimgearmeasuresTree->invisibleRootItem()->indexOfChild(matches[0]);
        swimgearmeasures.removeAt(index);
    } else {
        // add new
        index = swimgearmeasures.count();
        add = new QTreeWidgetItem;
        add->setFlags(add->flags() & ~Qt::ItemIsEditable);
        swimgearmeasuresTree->invisibleRootItem()->insertChild(index, add);
    }
    ...
    for (int k = 0; k < valuesEdit.count(); k++)
        addMeasure.values[k] = valuesEdit[k]->value();
    addMeasure.comment = comment->text();
    addMeasure.swimgearsource = SwimGearMeasure::Manual;
    addMeasure.swimgearoriginalSource = "";
    swimgearmeasures.insert(index, addMeasure);
...
void
SwimGearMeasuresPage::deleteClicked()
{
    if (swimgearmeasuresTree->currentItem()) {
        int index = swimgearmeasuresTree->invisibleRootItem()->indexOfChild(swimgearmeasuresTree->currentItem());
        delete swimgearmeasuresTree->invisibleRootItem()->takeChild(index);
        swimgearmeasures.removeAt(index);
    }
}

void
SwimGearMeasuresPage::rangeEdited()
{
    if (swimgearmeasuresTree->currentItem()) {
        int index = swimgearmeasuresTree->invisibleRootItem()->indexOfChild(swimgearmeasuresTree->currentItem());

        QDateTime dateTime = dateTimeEdit->dateTime();
        QDateTime odateTime = swimgearmeasures[index].when;

        bool valuesChanged = false;
        for (int k = 0; !valuesChanged && k < valuesEdit.count(); k++)
            if (valuesEdit[k]->value() != swimgearmeasures[index].values[k])
                valuesChanged = true;
        QString ncomment = comment->text();
        QString ocomment = swimgearmeasures[index].comment;
...
void
SwimGearMeasuresPage::rangeSelectionChanged()
{
    // fill with current details
    if (swimgearmeasuresTree->currentItem()) {

        int index = swimgearmeasuresTree->invisibleRootItem()->indexOfChild(swimgearmeasuresTree->currentItem());
        SwimGearMeasure current = swimgearmeasures[index];
