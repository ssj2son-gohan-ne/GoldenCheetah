...
// How to import ecxercises File|Streams
...
PolarFlow::PolarFlow(Context *context) : CloudService(context), context(context) {
    ...
    settings.insert(Local3, GC_POLARFLOW_STATUS_CODE);
    settings.insert(Local4, GC_POLARFLOW_ACTIVITY_COUNTER);
    settings.insert(Local5, GC_POLARFLOW_LAST_EXERCISE_ID);
    // settings.insert(Local5, GC_POLARFLOW_SAMPLE_SIZE); // samples - streams
    settings.insert(Local6, GC_POLARFLOW_TIMESTREAM_LOOPNUMBER); // samples - streams
    settings.insert(Local7, GC_POLARFLOW_TIMESTREAM_RECORDINGRATE); // samples - streams
...
PolarFlow::open(QStringList &errors) //open transaction
{
  ...
  // Command-URL: POST https://www.polaraccesslink.com/v3/users/{user-id}/exercise-transactions
    ...
    if (statusCode == 201) {
        ...
        printd("open - transaction_id_String: %s \n", transaction_id.toStdString().c_str());
...
PolarFlow::close()
{
    // which statements have to be full filled?
    //  - statusCode from open() has to be 204 no new content: https://www.polar.com/accesslink-api/?shell#create-transaction-2
    //  - activity counter from readDir needs to be <=50: https://www.polar.com/accesslink-api/?shell#training-data

    /*
    printd("PolarFlow::start\n");
    int statusCode = getSetting(GC_POLARFLOW_STATUS_CODE, "").toInt();
    int activityCounter = getSetting(GC_POLARFLOW_ACTIVITY_COUNTER, "").toInt();

    QStringList emptyerrorlist;
    if (statusCode != 204 && activityCounter <= 50) {
        printd("Status Code is not 204 (actual Status %d)\n", statusCode, activityCounter);
        return false;
    }
...
//PolarFlow::readdir(QString path, QStringList &errors, QDateTime from, QDateTime to)
{
 ...
 //Command URL: GET https://www.polaraccesslink.com/v3/users/{user-id}/exercise-transactions/{transaction-id}
    QString url;
    url = QString("%1/v3/users/%2/exercise-transactions/%3")
          .arg(getSetting(GC_POLARFLOW_URL, "https://www.polaraccesslink.com").toString())
          .arg(getSetting(GC_POLARFLOW_USER_ID, "").toString())
          .arg(getSetting(GC_POLARFLOW_TRANSACTION_ID, "").toString());
    ...
    for(int i = 0 ; i < exerciseUrls_Ary.size(); i++) {
        ...
        printd("readdir - loop i: %d (max=%d) exerciseUrl_String - Url to exercise: %s \n", loopCounter, activityCounterMax, exerciseUrl_Str.toStdString().c_str());
        ...
        // if successful, lets unpack
        int statusCode = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
        printd("readdir - loop i:%d - errornumber: %d - error text: %s\n", loopCounter, reply->error(), reply->errorString().toStdString().c_str());
        printd("readdir - loop i:%d - Exercise HTTP response status code: %d\n", loopCounter, statusCode);
        ...
        if(loopCounter == exerciseUrls_Ary.size()) {
            QString lastActivityId = QString::number(get_exercise_summary_Obj["id"].toInt());
            //int lastActivityId = get_exercise_summary_Obj["id"].toInt();
            setSetting(GC_POLARFLOW_LAST_EXERCISE_ID, lastActivityId);
            CloudServiceFactory::instance().saveSettings(this, context);
            printd("number of last loop: %d (max=%d) - last Excercise Id (saved): %s \n", loopCounter, activityCounterMax, lastActivityId.toStdString().c_str());
        }
        returning << add;
    }
    printd("readdir - loop end \n");

    // all good ?
    printd("readdir - closed \n");
...
PolarFlow::clearJsonObject(QJsonObject &object)
{
    ...
    printd("clearJsonObject - end\n");
}

QJsonObject
PolarFlow::createStreams(QJsonDocument &document, int loopnumber)
{
    printd("PolarFlow::createStreams - start \n");

    int timeStreamLoopNumber = getSetting(GC_POLARFLOW_TIMESTREAM_LOOPNUMBER, "").toInt();
    ...
    //{
    //  "recording-rate": 5,                                 // integer(int32)
    //  "sample-type": "1",                                  // string(byte)
    //  "data": "0,100,102,97,97,101,103,106,96,89,88,87,98" //string, comma-separated list
    //}
    ...
    if (loopnumber < timeStreamLoopNumber) {
      ...
      */
      ...
      printd("createStreams %s with id: %s found\n", samplename_Str.toStdString().c_str(), sampletype_id_Str.toStdString().c_str());
      ...
      printd("createStreams - recording_rate_String: %s\n", recording_rate_Str.toStdString().c_str());

      // create array from list of comma separeted e.g. heart rate values
      QString datalist_Str;
              datalist_Str = sampletype_Doc["data"].toString();
      printd("createStreams - datalist_String: %s\n", datalist_Str.toStdString().c_str());

      // create a list of all the data we will work with
      QStringList datalist_Lst;
                  datalist_Lst = datalist_Str.split(QLatin1Char(','));
      printd("createStreams - datalist_List no of elements: %d\n", datalist_Lst.size());
      ...
      //qDebug() << "createStreams - datalist_Ary:" << datalist_Ary;

      // save HR Sample size and recording for generating suitable timeStream
      if (sampletype_id_Str == "0") {
          setSetting(GC_POLARFLOW_SAMPLE_SIZE, datalist_Lst.size());
          setSetting(GC_POLARFLOW_TIMESTREAM_RECORDINGRATE, recording_rate_Str.toInt());
      }
      ...
      //qDebug() << "createStreams - stream_Object:" << sampleStream_Obj;
    ...
    if (loopnumber == timeStreamLoopNumber) {
        ...
        int recordingRate = getSetting(GC_POLARFLOW_TIMESTREAM_RECORDINGRATE, "").toInt();

        // convert integer to float
        //https://www.codegrepper.com/code-examples/cpp/int+to+float+c%2B%2B
        float recording_rate = (float)recordingRate;

        QList<float> timeFromRate_Lst;
                     //timeFromRate_Lst.append(0.0); // first member is zero
        for(int i=0; i<loops; i++) {
            float localLoopNumber = (float)i;
            float time = recording_rate*localLoopNumber;
            timeFromRate_Lst.append(time);
        }
        printd("createStreams - timeFromRate_List no of elements: %d \n", timeFromRate_Lst.size());
        //qDebug() << "createStreams - timeFromRate_List:" << timeFromRate_Lst;

        QJsonArray time_Ary;
                   //foreach(QList<int> data, timeFromRate_Lst)
                   for(int i=0; i<timeFromRate_Lst.size(); i++) {
                   time_Ary.append(timeFromRate_Lst.at(i));
                   }
        printd("createStreams - time_Array no of elements: %d \n", time_Ary.size());
        //qDebug() << "createStreams - time_Array:" << time_Ary;

        QJsonObject timeStream_Obj;
                    timeStream_Obj.insert("type", "time");
                    timeStream_Obj.insert("data", time_Ary);
                    timeStream_Obj.insert("series_type", "");
                    timeStream_Obj.insert("original_size", time_Ary.size());
                    timeStream_Obj.insert("recording_rate", recordingRate);

        //qDebug() << "createStreams - time_Object:" << timeStream_Obj;
...
{
    printd("PolarFlow::addSamples - start\n");
    ...
    QString get_available_samples_urls_Str;
            get_available_samples_urls_Str = QString("%1/v3/users/%2/exercise-transactions/%3/exercises/%4/samples")
               .arg(getSetting(GC_POLARFLOW_URL, "https://www.polaraccesslink.com").toString())
               .arg(getSetting(GC_POLARFLOW_USER_ID, "").toString())
               .arg(getSetting(GC_POLARFLOW_TRANSACTION_ID, "").toString())
               .arg(exerciseId);
    //printd("addSamples - URL-List Samples: %s\n", get_available_samples_urls_Str.toStdString().c_str());

    // request using the bearer token
    QNetworkRequest request(get_available_samples_urls_Str);
                    request.setRawHeader("Authorization", (QString("Bearer %1").arg(token)).toLatin1());
                    request.setRawHeader("Accept", "application/json");
    ...
    printd("addSamples - HTTP response status code: %d\n", statusCode);

    // Example: https://www.polaraccesslink.com/v3/users/{user-id}/exercise-transactions/{transaction-id}/exercises/{exercise-id}/samples/{type-id}
    // Returns a list of hyperlinks to available exercises
    QByteArray getAvailableSamplesUrls_Ary = reply->readAll();
    printd("page : %s\n", getAvailableSamplesUrls_Ary.toStdString().c_str());
    printd("addSamples - number of samples: %d\n", getAvailableSamplesUrls_Ary.size()); // size is nonsense, or amount signs in string
    printd("addSamples - QByteArray: %s\n", getAvailableSamplesUrls_Ary.toStdString().c_str());
    ...
    */
    ...
    // parse JSON payload
    QJsonParseError listgetAvailableSamples_Doc_parseError;
    QJsonDocument listgetAvailableSamples_Doc;
                  listgetAvailableSamples_Doc = QJsonDocument::fromJson(getAvailableSamplesUrls_Ary, &listgetAvailableSamples_Doc_parseError);
    //qDebug() << "addSamples - Document Error Number: " << listgetAvailableSamples_Doc_parseError.error << " - Document Error String: " << listgetAvailableSamples_Doc_parseError.errorString();
    //qDebug() << "addSamples - Document list of urls: " << listgetAvailableSamples_Doc;

    //[ {
    //  "type" : "heartrate",                                                       // string
    //  "data" : [ 0,100,102,97,97,101,103,106,96,89,88,87 ],                       // float? // integer?
    //  "series_type" : "0",                                                        // integer
    //  "original_size" : 12,                                                       // integer
    //  "recording_rate" : "1"                                                      // integer
    //  }
    //  {
    //  "type" : "distance",                                                        // string
    //  "data" : [ 2.9, 5.8, 8.5, 11.7, 15, 19, 23.2, 28, 32.8, 38.1, 43.8, 49.5 ], // float?
    //  "series_type" : "10",                                                       // integer
    //  "original_size" : 12,                                                       // integer
    //  "recording_rate" : "1"                                                      // integer
    //  }
    //]
    ...
    QJsonArray streams; // fetches all provided streams and adds time stream
    QJsonObject sampleStream; // filled with all available samples besides not provided timestream and added to QJsonArray streams
    QJsonObject timeStream; // timestream will be created seperatly, but gets number of datapoints from heartrate sample, with is always provided and added to QJsonArray streams
    int timeStreamLoopNumber = 0; // is set in heartrate stream, default is set to zero


    if (listgetAvailableSamples_Doc_parseError.error == QJsonParseError::NoError) {

        QJsonObject listgetAvailableSamples_Obj;
                    listgetAvailableSamples_Obj = listgetAvailableSamples_Doc.object();
        //qDebug() << "addSamples - Samples Exercise - Object url sample exercises: " << listgetAvailableSamples_Obj;

        QJsonArray samplesfromExercise_Ary;
                   samplesfromExercise_Ary = listgetAvailableSamples_Obj["samples"].toArray();

        //qDebug() << "addSamples - Samples Exercise - Array of Sample Urls: " << samplesfromExercise_Ary;

        // set timeStream Loop number
        // time stream is created after all other streams, see below
        setSetting(GC_POLARFLOW_TIMESTREAM_LOOPNUMBER, (samplesfromExercise_Ary.size()+1));

        // create time stream at the end and move it to first Position in Array
        timeStreamLoopNumber = getSetting(GC_POLARFLOW_TIMESTREAM_LOOPNUMBER, "").toInt();
        printd("addSamples - timestream - loopnumber: %d\n", timeStreamLoopNumber);
        ...
        for(int i = 0 ; i < samplesfromExercise_Ary.size(); i++) {
            int loopCounter = i + 1; // amount of Samples counts from one
            sampleUrl_Str = samplesfromExercise_Ary[i].toString();
            printd("addSamples - loop i: %d / %d - sampleUrl_String - Url to sample-series: %s \n",loopCounter, samplesfromExercise_Ary.size(), sampleUrl_Str.toStdString().c_str());

            // Sample URL is porvided in
            ...
            int statusCode = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
            printd("fetch response: %d: %s\n", reply->error(), reply->errorString().toStdString().c_str());
            printd("addSamples - loop i: %d / %d  - HTTP response status code: %d \n",loopCounter, samplesfromExercise_Ary.size(), statusCode);

            // no bracktes []
            QByteArray sampletype_Ary = reply->readAll();
            printd("addSamples - loop i: %d / %d - sampletype_Array: %s \n",loopCounter, samplesfromExercise_Ary.size(), sampletype_Ary.toStdString().c_str());

            // parse JSON payload
            QJsonParseError sampletype_Doc_parseError;
            QJsonDocument sampletype_Doc;
                           sampletype_Doc = QJsonDocument::fromJson(sampletype_Ary, &sampletype_Doc_parseError);
            //qDebug() << "addSamples - loop i:"<< loopCounter <<"/"<< samplesfromExercise_Ary.size() << " - sampletype_Document - Error Number:" << sampletype_Doc_parseError.error << " - sampletype_Document - Error String: " << sampletype_Doc_parseError.errorString();
            //qDebug() << "addSamples - loop i:"<< loopCounter <<"/"<< samplesfromExercise_Ary.size() << " - sampletype_Document:" << sampletype_Doc;

            if (sampletype_Doc_parseError.error == QJsonParseError::NoError) {
                sampleStream = createStreams(sampletype_Doc, i);
                //qDebug() << "addSamples - loop i:" << loopCounter <<"/"<< samplesfromExercise_Ary.size() <<" - sampleStream:" << sampleStream;
                streams.append(sampleStream);
                //qDebug() << "addSamples - loop i:" << loopCounter <<"/"<< samplesfromExercise_Ary.size() <<" - streams:" << streams;
            } // close doc parser error

        } // close get available Samples
        printd("addSamples - amount of streams (size) without timestream %d \n", streams.size());
        ...
        timeStream = createStreams(emptyForTimestream_Doc, timeStreamLoopNumber);
        // and prepend it as first stream in QJsonArray streams
        streams.prepend(timeStream);
        //printd("addSamples - streams with time: %s", streams);
        printd("addSamples - amount of streams (size) with timestream: %d / loopnumber: %d\n", streams.size(), timeStreamLoopNumber);
        ...
        static struct {
               ...
               { RideFile::secs,      "time",                  1.0f   }, // s (recording rate)
               ...
               { RideFile::rcad,       "runningCadence",       2.0f   }, // spm // conversation factor "2" same Value as Strava
        ...
        // define QList<polar_samples> syntax
        class polar_samples { // polar_samples = strava_stream
        public:
        ...
        // create a list of all the data we will work with
        QList<polar_samples> data_Lst;
        // Example: 1.0f; RideFile::hr; 0,100,102,97,97,101,103,106,96,89,88,87
        //qDebug() << "addSamples - Samples - loop i:" << loopCounter <<"- data: " << data_Lst;

        // examine the returned object and extract sample data
        for(int j=0; j<streams.size(); j++) {
            int jPlusOne = j + 1; // amount of Points counts from one
            QJsonObject stream = streams.at(j).toObject();
            QString type = stream["type"].toString();
            printd("addSamples - loop j: %d / %d - streams type: %s\n", jPlusOne, streams.size(), type.toStdString().c_str());

            for(int k=0; seriesnames[k].type != RideFile::none; k++) {
                QString name = seriesnames[k].polarsampletypename;
                printd("addSamples - loop k: %d - series name: %s\n", k, name.toStdString().c_str());
                polar_samples add;

                // contained?
                if (type == name) {
                    add.factor = seriesnames[k].factor;
                    add.type = seriesnames[k].type;
                    ...
                    //qDebug() << "addSamples - loop k: " << k << "- streams samples size:" << stream["data"].toArray().size();
                    //qDebug() << "addSamples - loop k: " << k << "- streams samples:" << stream["data"].toArray();

                    data_Lst << add;
                    //qDebug() << "addSamples - loop k: " << k << "- data_Lst : " << data_Lst;
                    break; // breaks to j loop
                  } // close type
            } // close stream data - loop k

        } // close streams size - loop j
        ...
        int dataPointsImport;
            dataPointsImport = ret->dataPoints().count();
            if (dataPointsImport != 0) {
                dataPointsImport = 0;
            }
        printd("addSamples - dataPointsImport %d:\n", dataPointsImport);

        int samplePoints;
            samplePoints = getSetting(GC_POLARFLOW_SAMPLE_SIZE, "").toInt();
        printd("addSamples - samplePoints     %d:\n", samplePoints);

        polar_samples stream;
        int streamPoints;
            streamPoints = stream.samples.count();


        printd("addSamples - streamPoints (Zero at this point): %d\n", streamPoints);
        ...
        if (ret->isDataPresent(RideFile::hr) && numberOfPointsAreEven == true ) {
            ...
            int indexEnd = 0; // amount of datapoints in ONE stream
            ...
            int streamType = 1;
            int streamTypeDelta = 1; // decreasing by one
            ...
            printd("addSamples - countSamplesEnd: %d\n", countSamplesEnd);
            printd("addSamples - indexEnd: %d\n", indexEnd);

            //Create empty Lists for collecting values from factorized sample data
            QList<double> streamTypeSecs;
            QList<double> streamTypeLat;
            QList<double> streamTypeLon;
            QList<double> streamTypeHr;
            QList<double> streamTypeKph;
            QList<double> streamTypeCad;
            QList<double> streamTypeAlt;
            QList<double> streamTypeWatts;
            QList<double> streamTypePowerpedalingindex;
            QList<double> streamTypeLRBalance;
            QList<double> streamTypeRcad;
            QList<double> streamTypeTemp;
            QList<double> streamTypeKm;
            QList<double> streamTypeRRInterval;

            // define stop conditions
            streamSize = streams.size();
            //if (ret->isDataPresent(RideFile::lat) && ret->isDataPresent(RideFile::lon)) {
            //streamSize = streamSize + 2;
            //}
            //if (stream.type == "rrIntervall") {
               ...
               foreach(stream, data_Lst) {
                 ...
                 if (stream.type == RideFile::secs) {
                    ...
                    //if (stream.type == RideFile::secs) {
                         ...
                         if (ret->isDataPresent(RideFile::secs)) {
                            ...
                            //value = value;
                            printd("addSamples - imported choosen\n");
                         }
                         if (!(ret->isDataPresent(RideFile::secs))) {
                             value = stream.factor * stream.samples.at(index).toDouble();
                             printd("addSamples - sample choosen\n");
                         }
                         streamTypeSecs.append(value);
                         //streamTypeSecs.replace(index, value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Secs countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }
                 ...
                 if (stream.type == RideFile::lat) {
                     if (countSamples < countSamplesEnd) {
                        ...
                        streamTypeLat.append(value);
                        //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Lat countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd << "- add stream.type:" << stream.type << ", value:" << value;
                    }
                 }

                 if (stream.type == RideFile::lon) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeLon.append(value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Lon countSamplesPlusOne:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 if (stream.type == RideFile::hr) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeHr.append(value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- HR countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 if (stream.type == RideFile::kph) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeKph.append(value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Kph countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 if (stream.type == RideFile::cad) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeCad.append(value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Cad countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 if (stream.type == RideFile::alt) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeAlt.append(value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Alt countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 if (stream.type == RideFile::watts) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeWatts.append(value);
                        //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Watts countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 if (stream.type == RideFile::rcad) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeRcad.append(value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Rcad countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 if (stream.type == RideFile::lrbalance) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeLRBalance.append(value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- lrBalance countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 if (stream.type == RideFile::temp) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeTemp.append(value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Temp countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd  << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 if (stream.type == RideFile::km) {
                     if (countSamples < countSamplesEnd) {
                         countSamplesPlusOne = countSamples + 1;
                         value = valueStart;
                         ...
                         streamTypeKm.append(value);
                         //qDebug() << "addSamples - index:"<< indexPlusOne << "/" << indexEnd << "- Km countSamples:"<< countSamplesPlusOne << "/" << countSamplesEnd << "- add stream.type:" << stream.type << ", value:" << value;
                     }
                 }

                 //value = valueStart;
                 printd("addSamples - index: %d / %d - countSamples - next round\n", indexPlusOne, indexEnd);
                 //}
                 // loopnumber decrease
                ...
                if (countSamples == countSamplesEnd) {
                    //countSamples = 0;
                    printd("addSamples - index: %d / %d - countedSamples: %d / %d \n", indexPlusOne, indexEnd, countSamplesPlusOne, countSamplesEnd);
                    //qDebug() << "addSamples - QList - Secs data:" <<streamTypeSecs;
                    //qDebug() << "addSamples - QList - Hr data:" <<streamTypeHr;
                    //qDebug() << "addSamples - QList - Lat data:" <<streamTypeLat;
                    printd("addSamples - QList - Secs size: %d \n", streamTypeSecs.size());
                    printd("addSamples - QList - Hr size: %d \n", streamTypeHr.size());
                    printd("addSamples - QList - Lat size: %d \n", streamTypeLat.size());
                    printd("addSamples - QList - Lon size: %d \n", streamTypeLon.size());
                    printd("addSamples - QList - Kph size: %d \n", streamTypeKph.size());
                    printd("addSamples - QList - Cad size: %d \n", streamTypeCad.size());
                    printd("addSamples - QList - Alt size: %d \n", streamTypeAlt.size());
                    printd("addSamples - QList - Watts size: %d \n", streamTypeWatts.size());
                    printd("addSamples - QList - Powerpedalingindex size: %d \n", streamTypePowerpedalingindex.size());
                    printd("addSamples - QList - LRBalance size: %d \n", streamTypeLRBalance.size());
                    printd("addSamples - QList - Rcad size: %d \n", streamTypeRcad.size());
                    printd("addSamples - QList - Temp size: %d \n", streamTypeTemp.size());
                    printd("addSamples - QList - Km size: %d \n", streamTypeKm.size());
                } // close if countSamples counter

               } // close foreach-loop
            } // if index loop - filling QLists<streamType*>

            // Sorting in Update and afterwards remove stream.type
            printd("addSamples - before appendOrUpdate countSamples: %d \n", countSamples);
...
/*
                ...
                ret->appendOrUpdatePoint(
                         // double secs,              double cad,              double hr,              double km,
                            if (streamTypeSecs.size()>0) streamTypeSecs.at(index),
                            streamTypeCad.at(index),
                            streamTypeHr.at(index),
                            streamTypeKm.at(index),
                         // double kph,              double nm,     double watts,              double alt,
                            streamTypeKph.at(index), RideFile::NIL, streamTypeWatts.at(index), streamTypeAlt.at(index),
                         // double lon,              double lat,              double headwind,
                            streamTypeLon.at(index), streamTypeLat.at(index), RideFile::NIL,
                         // double slope,  double temp,              double lrbalance,
                            RideFile::NIL, streamTypeTemp.at(index), streamTypeLRBalance.at(index),
                         ...
                         // double rvert,  double rcad,            double rcontact, double tcore,
                            RideFile::NIL, streamTypeRcad.at(index), RideFile::NIL, RideFile::NIL,
                         // int interval, bool forceAppend
                            0, false);
*/
                //qDebug() << "addSamples - appendOrUpdate"<< indexPlusOne << "/" << indexEnd << "- ret:" << ret;
                printd("addSamples - appendOrUpdate - End \n");
        ...
        if (isImported == false) {
            bool end = false;
            int index=0;
            printd("addSamples - do-while data List size / available Streams: %d \n", data_Lst.size());

            do {
                RideFilePoint add;

                // move through streams if they're waiting for this point
                foreach(polar_samples stream, data_Lst) {
...
/*
                    if (stream.type == RideFile::lat) {
                        ...
                        lat = stream.factor * stream.samples.at(index).toArray().at(0).toDouble();
                        lon = stream.factor * stream.samples.at(index).toArray().at(1).toDouble();
...
//                      }
                    //qDebug() << "addSamples - do-while"<< index << "/" << stream.samples.count()  << "- else add stream.type, value:" << stream.type << ", " << value;
                  } else
                      end = true;
                }

                ret->appendPoint(add); // example: ret->appendPoint(RideFile::hr, 182)
                //printd("addSamples - do-while streamSize %d \n", stream.size());
                //qDebug() << "addSamples - do-while" << index << "- ret:" << ret;
                index++;

            } while (!end);
            printd("addSamples - do-while - number of samples per stream: %d \n", index); // << "- ret:" << ret;
...
PolarFlow::prepareResponse(QByteArray* data, QString &name)
{
    ...
    QString typeOfAcquiring; // "File|Streams"
            typeOfAcquiring = POLARFLOW_IMPORTTYPE;
            //typeOfAcquiring = "File";
    ...
    // Acquire Excercise via File-Import
    if (typeOfAcquiring == "File") {
        // get detailed-sport-info
        ...
        QString filenameWithPathTCX;
                filetype = "tcx";
                filenameWithPathTCX = getFile(exercise_id_Str, filetype, starttime, detailedSportInfo_Str);

        QString filenameWithPathGPX;
                filetype = "gpx";
                filenameWithPathGPX = getFile(exercise_id_Str, filetype, starttime, detailedSportInfo_Str);

        QString filenameWithPathFITBeta;
                filetype = "fit";
                filenameWithPathFITBeta = getFile(exercise_id_Str, filetype, starttime, detailedSportInfo_Str);
        ...
        QString filenameWithPathSuffix;
                filenameWithPathSuffix = QFileInfo(filenameForImport).suffix();
        ...
        // minimal information to add to imported file, so we can see where we got the information from
        // Set Source Filename
    ...
    if (typeOfAcquiring == "Streams") {
        ...
        */
        ...
        //printd("prepareResponse - reply:%s\n", data->toStdString().c_str());
        printd("prepareResponse - end \n");
    } // close lets set root

    // if we have aquired the last exercise from list, so we can commit the transaction
    QStringList emptyerrorlist;
    QString lastActivityId = getSetting(GC_POLARFLOW_LAST_EXERCISE_ID, "").toString();
    printd("prepareResponse - last exercise id: %s , this exercise id: %s \n", lastActivityId.toStdString().c_str(), exercise_id_Str.toStdString().c_str());
    if (lastActivityId != exercise_id_Str) {
        printd("prepareResponse - not calling commit - not last exercise id: %s \n", lastActivityId.toStdString().c_str());
    }
    else {
        printd("prepareResponse - calling commit - last exercise id: %s \n", lastActivityId.toStdString().c_str());
        commit(emptyerrorlist);
